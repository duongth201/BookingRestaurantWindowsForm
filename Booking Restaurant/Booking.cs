...
namespace Booking_Restaurant
{
    public partial class Booking : Form
    {
        ...
        public void bindGrid(bool a)
        {
            ...
            //4timeid not present table order
            int timeid = int.Parse(comboBox1.SelectedValue.ToString());
            var fill = context.Bookings.Where(x => x.Orderdate == (DateTime)dateTimePicker1.Value).Where(x => x.TimeId > timeid - 4 && x.TimeId < timeid + 4).Select(x => x.TableId).ToList();

            var data = context.Tables.Where(x => !fill.Contains(x.Id)).Where(x => x.Capacity >= people).ToList();
